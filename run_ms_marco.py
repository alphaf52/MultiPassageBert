...
from ms_marco_data_reader import MSMarcoDataset, MSMarcoIterDataset

from transformers import WEIGHTS_NAME, BertConfig, BertTokenizer
from transformers import AdamW, WarmupLinearSchedule

from utils_ms_marco import RawResult, write_predictions
from ms_marco_evaluation.ms_marco_eval import *
...
def evaluate(args, model, tokenizer, prefix=""):
    dataset = MSMarcoDataset(args.predict_file, subset='dev')
    ...
    eval_dataloader = DataLoader(dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, num_workers=args.n_gpu)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            inputs = {'input_ids':       input_ids.view(-1, input_ids.shape[2]),
                      ...
                      'token_type_ids':  segment_ids.view(-1, segment_ids.shape[2])}
            outputs = model(**inputs)
            start_logits, end_logits = outputs[0], outputs[1] # (batch_size * num_para, max_seq_len)
            start_logits = start_logits.view(batch_size, args.num_para * args.max_seq_length)
            end_logits = end_logits.view(batch_size, args.num_para * args.max_seq_length)

        for i, example_index in enumerate(example_indices):
            result = RawResult(index        = example_index,
    ...
    results = compute_metrics_from_files(args.reference_file, output_prediction_file, MAX_BLEU_ORDER)
    return results
...
def main():
    ...
    parser.add_argument("--train_file", default=None, type=str, required=True,
                        help="cached feature file for training set")
    parser.add_argument("--predict_file", default=None, type=str, required=True,
                        help="cached feature file for dev / eval set")
    parser.add_argument("--reference_file", default=None, type=str, required=True,
                        help="reference output for dev / eval set")
    ...
    parser.add_argument("--num_para", default=10, type=int,
    ...
    parser.add_argument("--gamma", default=1.0, type=float,
                        help="Weight for span prediction loss.")
    ...
    # Training
    if args.do_train:
        train_dataset = MSMarcoDataset(args.train_file, subset='train')
