...
def write_predictions(all_features, all_results, eval_qid_set,
    ...
    for (index, result) in enumerate(all_results):
        ...
        if not version_2_with_negative:
            all_predictions[qid] = nbest_json[0]["text"]
        else:
            # predict "" iff the null score - the score of best non-null > threshold
            score_diff = score_null - best_non_null_entry.start_logit - (
                best_non_null_entry.end_logit)
            scores_diff_json[qid] = score_diff
            if score_diff > null_score_diff_threshold:
                all_predictions[qid] = "No Answer Present."
            else:
                all_predictions[qid] = best_non_null_entry.text
        all_nbest_json[qid] = nbest_json
